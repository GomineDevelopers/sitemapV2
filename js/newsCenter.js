var newsCenter = new Vue({
    ...
    mounted: function() {
        ...
        axios.all([getLeft(), getRight()]).then(
            axios.spread(function(leftC, rightC) {
                // 两个请求现在都执行完成
                vm.loading = false;
    ...
    methods: {
        ...
        Search: function() {
            ...
            getLeft(vm.cur, vm.keyWord).then(function(response) {
                vm.loading = false;
        ...
        btnClick: function(data) {
            ...
            if (data != this.cur) {
                this.cur = data;
                getLeft(this.cur, vm.keyWord).then(function(response) {
                    vm.loading = false;
                    vm.newsLeftData = response.data.data.data;
                    vm.newsLeftData.forEach(function(element, index, array) {
                        // element: 指向当前元素的值 index: 指向当前索引 array: 指向Array对象本身
...
function getRight() {
    return axios.get(globalUrl + "content/popularlist");
}
