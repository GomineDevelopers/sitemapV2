...
if(JSON.parse(localStorage.getItem("type")).type==0){
    if(key= JSON.parse(localStorage.getItem("searchLitToMap")).key){
        var storage={}
    ...
    function addMarker() {
        ...
        $.ajax({
            url: globalUrl + "content/gisstate/",
            type: "GET",
            dataType: "json",
            data: {
                info: info,
    ...
    function addClickHandler(marker, sign) {
        ...
        marker.addEventListener("click", function(e) {
            ...
            myGeo.getLocation(newPoint, function(result) {
                ...
                $.ajax({
                    url: globalUrl + "content/giscitys/",
                    type: "GET",
                    dataType: "json",
                    data: {
                        info: info,
        ...
        $.ajax({
            ...
            success: function (data, status) {
                ...
                for (var i = 0; i < data.data.length; i++) {
                    ...
                    mouseHoverEvent(marker, data.data[i]);
                    mouseOutEvent(marker);
                    var point2 = new BMap.Point(116.405275,39.924785);
                    var circle = new BMap.Circle(point2,5000,{strokeColor:"#f5533d", strokeWeight:2, strokeOpacity:0.5});
